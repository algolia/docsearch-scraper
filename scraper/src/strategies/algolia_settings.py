    ...
    def get(config, levels):
        ...
        for level in levels:
            for selectors_key in config.selectors:
                ...
                attr_to_index_whole = 'unordered(hierarchy.' + level + ')'
                if level in config.selectors[
                    selectors_key] and attr_to_index_whole not in attributes_to_index:
                    attributes_to_index.append(
                        'unordered(hierarchy_camel.' + level + ')')
                    attributes_to_index.append(attr_to_index_whole)
