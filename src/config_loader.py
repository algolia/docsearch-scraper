...
class ConfigLoader(object):
    ...
    def assert_config(user_data):
        ...
        # `download_timeout` is set to 10s by default unless it is specified
        if isinstance(data.get('download_timeout'), int):
            data['download_timeout'] = data.get('download_timeout')
        else:
            data['download_timeout'] = 10
