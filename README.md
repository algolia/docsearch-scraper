...
internal proxy (PhantomJS) to render pages before crawling them. To start our
PhantomJS proxy server, just type: `node docker/server.js`.
...
webpage with the PhantomJS proxy.

This parameter is optional and is set to `0`s by default.

### `download_timeout`

Sometimes, documentations have too many links. Because of the asynchronicity
and default timeout of the request processing, it may leads to pages which
aren't crawled. In this case, the `download_timeout` parameter is used to
increase the time to wait for each page before timeouting.

This parameter is optional and is set to `10`s by default.
...
handle those documentations, this image which embeds our PhantomJS proxy in
...
see [`js_render`](#js_render). If not, the PhantomJS instance won't be started.
